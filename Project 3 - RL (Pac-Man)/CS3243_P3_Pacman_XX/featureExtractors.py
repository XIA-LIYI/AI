    ...
    def getFeatures(self, state, action):
        ...
        for i in range(len(ghosts)):
            if state.getGhostStates()[i].scaredTimer > 0:
                scared_ghosts.append(ghosts[i])
                scared_ghostsGrid[int(ghosts[i][0])][int(ghosts[i][1])] = True
            else:
                not_scared_ghosts.append(ghosts[i])
                scared_ghostsGrid[int(ghosts[i][0])][int(ghosts[i][1])] = True
        ...
        if len(not_scared_ghosts) > 0:
            # if there is ghosts around, add the capule feature
            features["#-of-ghosts-1-step-away"] = sum((next_x, next_y) in Actions.getLegalNeighbors(g, walls) for g in not_scared_ghosts)
        ...
        if len(not_scared_ghosts) == 0 and food[next_x][next_y]:
            features["eats-food"] = 1.0
