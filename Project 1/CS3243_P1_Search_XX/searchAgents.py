...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if (currentPosition in self.corners) and (currentPosition not in exploredCorners):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            for corner in state[1]:
                corners.append(corner) # We must use deep copy.
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
...
def cornersHeuristic1(state, problem):
    ...
    if unexploredCorners:
        for corner in unexploredCorners:
            ...
            searchProblem = PositionSearchProblem(gameState=problem.gameState, start=current, goal=corner, warn=False, visualize=False)
            dist = len(search.ucs(searchProblem)) # UCS and BFS are both ok. But dfs is not acceptable.
...
def cornersHeuristic(state, problem):
    ...
    while unexploredCorners:
        # Use search algorithm (ManhattanDistance) to compute the distance
        hCost, corner = min([(util.manhattanDistance(current, corner), corner) for corner in unexploredCorners])       
